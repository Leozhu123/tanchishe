...
protected:
	const int gameSize = 20; //游戏区域大小
	int score = 0;
	int speed = 1;           //当前速度在第几档
	bool died = 0, win = 0;
...
public:
	...
	void run() {
		while (!(died || win)) {              //既没死也没赢
			...
			if (score == 397) win = 1;        //有谁真的能这么有耐心吗
			if (died) draw("died");
			if (win) draw("win");
		}
	}
...
int Classic_mode::receiveCommand() {
	const int SPEED_LV[6] = { 60,40,100,15,5,4}; //共有4档速度 
	bool got = 0;
	int command;
	...
	for (int i = 1; i <= SPEED_LV[speed]; ++i) {
		...
		if (_kbhit()) {
			...
			if (command == SPEEDUP && speed < 5) { ++speed; continue; }       //加速
			if (command == SPEEDDOWN && speed > 0) { --speed; continue; }     //减速
			if (command == PAUSE) {                                           //暂停
...
void Classic_mode::drawInterface() { //画游戏界面
	draw("gameInterface");
	gotoxy(51, 5); std::cout << "经典模式"; //其实也就这一处有些特别...吧...
	std::cout << std::flush;
}
...
void Classic_mode::draw(const std::string& obj) { //万能画笔。这种定义形参的方式灵感来源于system函数
	...
	if (obj == "helpBox") {
		...
		gotoxy(51, 9); cout << "W:向上移动";
		gotoxy(51, 10); cout << "S:向下移动";
		gotoxy(51, 11);	cout << "A:向左移动";
		gotoxy(51, 12); cout << "D:向右移动";
		gotoxy(51, 13); cout << "J:加速";
		gotoxy(51, 14);	cout << "K:减速";
		gotoxy(51, 15);	cout << "[空格] 暂停";
		gotoxy(49, 17);	cout << "Produced By GKWuxi";
		gotoxy(49, 18);	cout << "无锡市大桥实验学校";
		cout << flush;
	}
